...
<link rel="stylesheet" type="text/css" href="<?php echo base_url()?>css/jquery-ui-1.10.0.custom.min.css"/>
			...
			<div class='sliderLg floatR'>
				<div class='right-inner clearfix'>
					<?php if($bt->bsd_status == 10):?>
					<p class="full-width-link">Current order status is "In Progress". <br>You cannot cancel order. <br>Please be in touch with the supplier or OceanTailer Rep to have it cancelled.</p>
					<?php endif;?>
					<!-- First Row Container-->
...
<!-- 									<td width=15>Shipping Fee</td> -->
				...
				<?php if($bt->bsd_status != 1 && $bt->bsd_status != -1 && $bt->bsd_status != 10) {?>
				<div class='right-inner clearfix'>
					<!-- First Row Container-->
					<div class="topBrands searching-for">
						<div class="topBrandsHeader">
					...
					<script type="text/javascript">
						function cancelOrder(){
							if (confirm('are you sure to cancel the order? \n press [OK] to send cancel order or press [Cancel] to cancel order?')){
						      	...
						      	$.post('<?php echo base_url()?>buyer/orderUpdate',{action:'cancel',reason:reason,bsd_id:<?php echo $bt->bsd_id ?>,'<?php echo $this->security->get_csrf_token_name()?>':'<?php echo $this->security->get_csrf_hash()?>'},function(){
