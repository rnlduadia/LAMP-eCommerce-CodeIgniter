<?php if($type == 'bsd'){ ?>
	<div class='message-main-cont'>
		...
		<div class='message-det clearfix'>
			<h3 class='fl'>Subject: <?php echo $message->bsm_subject ?></h3>

				<?php if($message->bsm_supplier_read == 0){?>
				<p class='notify-new fr'>
					New!
				</p>

					<?php $this->users->update_unread_message($message->bsm_id,2);  ?>
				<?php }?>
			...
			<div class='message-inner-cont clearfix'>
				...
				<div class='clear'></div>
				<div class='fr'>
					<p class='small-p-label'><?php echo  $message->bsm_time; ?></p>
				</div>
			</div>
			...
			<div>
				...
				<div id='append-reply<?php echo $message->bsm_id?>'  class='clearfix reply-main-container'>
					<?php if(count($replies) == 0){?>
						<center><p>--No Reply Message--</p></center>
					<?php } else{ foreach($replies as $reply){?>
							<div class='clearfix'>
								...
								<div class='clear'></div>
								<div class='fr'>
									<p class='small-p-label'><?php echo $reply->bsr_time;?></p>
								</div>
							</div>
						<?php }?>
