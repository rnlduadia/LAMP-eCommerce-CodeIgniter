...
<!-- 									<td width=15>Shipping Fee</td> -->
								...
								foreach($btd as $btd_det){
									...
									<tr>
										...
										<td>-$<?php echo  number_format($deduction = $btd_det->btd_subamount * ($btd_det->btd_productFee/100),2); ?></td>
										<td>$<?php echo  number_format($net = $btd_det->btd_subamount - $deduction,2);  ?></td>
										<?php
								...
								<tr>
									...
									<td colspan='1' class='big-font'><b><label class='label-infoformat'>$<?php echo  number_format($bt->bsd_total_paymet,2) ?></label>&#160;&#160;<label id='shipping-label' class='label-infoformat'>+&#160;$<?php echo $shipping_total?></label></b></td>
						...
						function submit_refund(){
							if (confirm('are you sure to refund the order? \n press [OK] to send refund order or press [Cancel] to cancel refund order?'))
							{
								...
								$.post('<?php echo base_url()?>supplier/refund/<?php echo $bt->bsd_id ?>',{
									...
									bsd:<?php echo $bt->bsd_id ?>,action:'save','<?php echo $this->security->get_csrf_token_name()?>':'<?php echo $this->security->get_csrf_hash()?>'},
									function(value){
