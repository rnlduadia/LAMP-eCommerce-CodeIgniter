				...
				<div class='right-inner clearfix'>
					...
					<div class='product-cont padded-cont clearfix'>
										  ...
										  <ul class="slides">
										    ...
										    <li>
										    	<a onclick='return false' href="<?php echo base_url().$img->ii_link?>">
										    		<img width=240 src="<?php echo base_url().$img->ii_link?>" id="slide0" />
										    	</a>
										    </li>
										    <?php }?>
										  </ul>
						...
						<div class="fl half">
							...
							<div class="clear"> </div>
							<input type="text" id='ship_cost' value="<?php echo $inventory->ic_ship_cost_in_db ?>" />
						</div>

						<div class="fl half">
							<label for='min_order'>Minimum order</label>
							<div class="clear"> </div>
							<input type="text" id='min_order' value="<?php echo $inventory->ic_min_order ?>" />
						</div>
						...
						<button class='greenbutton floatR' id='save-update-stock'>Update</button>
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$('#save-update-stock').click(function(){
			...
			$.post('<?php echo base_url()?>inventory/update',{sku:sku,quan:quan,price:price,ret_price:ret_price,ic_id:ic_id,min_order:min_order,lead_time:lead_time, ship_cost:ship_cost,ship_from:ship_from,map:map,prom_text:prom_text,
				action:'update_stock','<?php echo $this->security->get_csrf_token_name()?>':'<?php echo $this->security->get_csrf_hash()?>'},function(result){
