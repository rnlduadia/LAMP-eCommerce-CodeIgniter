...
$("#submitLogin").on("click", function(){
    ...
    if(!userName || !password){
        alert("You left required fields blank.");
       		...
       		$.post("http://"+ defaultPageSettings.site +"/user/login",{name:userName,pass:password}, function(result){
		var  convert = JSON.parse(result);
	            if(convert.result == 1)
	            {
                        $("#frmLogin").attr("action","http://"+ defaultPageSettings.site+"/user/login_and_refresh");
                        $("#frmLogin").submit();
                        //$(window.event).attr('returnValue',false);
                        //$(window.location).attr('href', "http://"+ defaultPageSettings.site +"/buyer");//redirect to the user page
...
$("#sendMessage").on("click", function(){
    ...
    $.post("http://"+ defaultPageSettings.site +"/user/sendcontact", {
        contactName : name,
        contactEmail : email,
        contactPhone : phone,
...
$('#submitForgotPass').click(function(){
		...
		$.post("http://"+ defaultPageSettings.site +"/user/forgot",{email:email},function(result){
			//alert(result);
			//return false;
			var  convert = JSON.parse(result);
			//alert()
